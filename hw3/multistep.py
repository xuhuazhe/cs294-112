...
def main_atari(env, model, n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None, vf_type='nn', tau=0.1, timesteps_threshold=5000):
    ...
    channel  = env.observation_space.shape[2]
    num_actions = env.action_space.n
    logz.configure_output_dir(logdir)

    # Symbolic variables have the prefix sy_, to distinguish them from the numerical values
    # that are computed later in these function
    sy_ob_no = tf.placeholder(shape=[None, ob_dim_1, ob_dim_2, channel], name="ob", dtype=tf.float32) # batch of observations
    ...
    # value training
    #sy_path_obs = tf.placeholder(shape=[None, ob_dim_1, ob_dim_2, channel], name="path_ob", dtype=tf.float32)
    ...
    sy_logits_na, sy_value = model(sy_ob_no, num_actions, scope='policy', reuse=False)
    ...
    sy_wt_upper =  tf.exp(sy_adv_n/tau- sy_logprob_n) * sy_logprob_n
    sy_wt_lower =  tf.reduce_sum(tf.exp(sy_adv_n/tau- sy_logprob_n))
    sy_wt       =  sy_wt_upper/sy_wt_lower
    sy_urex_surr     =  tf.reduce_mean(sy_logprob_n * (sy_adv_n + tau * sy_wt))
    sy_return = tf.placeholder(shape=[None], dtype=tf.float32)
    sy_value_loss = (sy_value - sy_return)*(sy_value - sy_return)
    ...
    update_op_urex = tf.train.AdamOptimizer(sy_stepsize).minimize(sy_urex_surr)
    update_op_value = tf.train.AdamOptimizer(sy_stepsize).minimize(sy_value_loss)
    ...
    for i in range(n_iter):
        ...
        if total_timesteps > timesteps_threshold:
            if FLAGS.multistep_replay:
                index = np.random.sample(xrange(1, len(replay_path)), 5)
                paths = replay_path[index]

            vtargs, vpreds, advs = [], [], []
            for path in paths:
                rew_t = path["reward"]
                return_t = discount(rew_t, gamma)
                #_, vpred_t = model(path["observation"], num_actions, scope='policy', reuse=True)
                vpred_t = sess.run(sy_value, feed_dict={sy_ob_no:path["observation"]})
                vpred_t = np.squeeze(vpred_t)
                #vpred_t = np.array(vpred_t[0]
                #print('****************' * 2)
            ...
            _ = sess.run([update_op_value], feed_dict={sy_ob_no:ob_no, sy_return:vtarg_n, sy_stepsize: stepsize})
            #vf.fit(ob_no, vtarg_n) # TODO: train this again

            # Policy update
            if FLAGS.multistep_urex:
                _, oldlogits_na = sess.run([update_op_urex, sy_logits_na],
...
def run(env, model):
    ...
    main_atari(env, model, logdir='./link_data/log/cartpole-nn', vf_type='nn',
                animate=False)  # when you want to start collecting results, set the logdir
